...
class GeneticAlgorithm(object):
    ...
    def step(self):
        ...
        #       YOUR CODE HERE      #
        #############################
        novaPopulacija = self.bestN(self.keep)
        while len(novaPopulacija) < self.populationSize:
            otac, majka = self.selectParents()
            dijete = self.crossover(otac, majka)
            kromosom = np.array(self.mutate(dijete))
            dobrota = self.calculateFitness(kromosom)
            novaPopulacija.append((kromosom, dobrota))
        self.population = novaPopulacija
        self.population.sort(key=lambda t: -t[1])

        if (self.i > self.numIter) or (self.best() < self.e):
            zaustavi = True
        else:
            zaustavi = False

        return zaustavi, self.i, self.best()[0]
    ...
    def selectParents(self):
        ...
        #       YOUR CODE HERE      #
        #############################
        otac, majka = self.population[0][0], self.population[1][0]

        sumaDobrote = 0
        # varijabla info je oblika: [[kromosomi], dobrota]
        for info in self.population:
            dobrota = info[1]
            sumaDobrote += dobrota

        pokazivacO = np.random.uniform(0, sumaDobrote)
        pokazivacM = np.random.uniform(0, sumaDobrote)
        trenutnaDobrotaO, trenutnaDobrotaM = 0, 0

        for kromosom, dobrota in self.population:
            trenutnaDobrotaO += dobrota
            if trenutnaDobrotaO > pokazivacO:
                otac = kromosom
                break
        for kromosom, dobrota in self.population:
            trenutnaDobrotaM += dobrota
            if trenutnaDobrotaM > pokazivacM:
                majka = kromosom
                break
