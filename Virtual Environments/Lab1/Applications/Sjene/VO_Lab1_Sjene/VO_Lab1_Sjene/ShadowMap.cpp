...
LPDIRECT3DTEXTURE9              g_pFloorMap = NULL; 
...
INT WINAPI wWinMain( HINSTANCE, HINSTANCE, LPWSTR, int )
{
    ...
    D3DXVECTOR3 vFromPt = D3DXVECTOR3( 0.0f, 3.0f, -15.0f );
    ...
    g_Light.Diffuse.r = 0.7f;
    g_Light.Diffuse.g = 0.7f;
    g_Light.Diffuse.b = 0.7f;
    g_Light.Diffuse.a = 0.2f;
                                 ...
                                 void* pUserContext )
{
    ...
    // Read Map Texture from file
    V_RETURN(D3DXCreateTextureFromFile(pd3dDevice, L".\\ShadowMap.jpg", &g_pFloorMap));
    ...
    V_RETURN( DXUTFindDXSDKMediaFileCch( str, MAX_PATH, L"Models\\spotlight.x" ) );
                                ...
                                const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    *( LPDWORD )lr.pBits = D3DCOLOR_RGBA( 255, 255, 255, 255 );
    ...
    V_RETURN( g_pEffect->SetVector( "light.diffuse", ( D3DXVECTOR4* )&g_Light.Diffuse ) );
    V_RETURN( g_pEffect->SetVector( "light.ambient", ( D3DXVECTOR4* )&g_Light.Ambient ) );
    V_RETURN( g_pEffect->SetFloat( "light.cosTheta", cosf( g_Light.Theta ) ) );
...
void CALLBACK OnFrameMove( double fTime, float fElapsedTime, void* pUserContext )
{
    ...
    D3DXMatrixMultiply( &g_Obj[1].m_mWorld, &m, &g_Obj[1].m_mWorld );
	/*
	D3DXMatrixRotationY( &m, -D3DX_PI * fElapsedTime / 4.0f );
    D3DXMatrixMultiply( &g_Obj[2].m_mWorld, &m, &g_Obj[2].m_mWorld );
	*/
	/*
    D3DXMatrixRotationY( &m, -D3DX_PI * fElapsedTime / 4.0f );
                  ...
                  const D3DXMATRIX* pmProj )
{
    ...
    V( g_pEffect->SetMatrix( "projMatrix", pmProj ) );
    ...
    if( g_bFreeLight )
    {
        ...
        V( g_pEffect->SetVector( "light.pos", &v4 ) );
        *( D3DXVECTOR3* )&v4 = *g_LCamera.GetWorldAhead();
        ...
        V( g_pEffect->SetVector( "light.dir", &v4 ) );
    }
    else
    {
        ...
        D3DXVec3Normalize( ( D3DXVECTOR3* )&v4, ( D3DXVECTOR3* )&v4 );
        V( g_pEffect->SetVector( "light.dir", &v4 ) );
        vPos += v4 * -52.0f;  // Offset the center by 3 so that it's closer to the headlight.
        V( g_pEffect->SetVector( "light.pos", &vPos ) );
    }
    ...
    if( SUCCEEDED( pd3dDevice->BeginScene() ) )
    {
        ...
        for( int obj = 0; obj < NUM_OBJ; ++obj )
        {
            ...
            V( g_pEffect->SetMatrix( "worldViewMatrix", &mWorldView ) );
            ...
            for( UINT p = 0; p < cPass; ++p )
            {
                ...
                for( DWORD i = 0; i < g_Obj[obj].m_Mesh.m_dwNumMaterials; ++i )
                {
                    ...
                    V( g_pEffect->SetVector( "material", &vDif ) );
                    if( g_Obj[obj].m_Mesh.m_pTextures[i] )
                        V( g_pEffect->SetTexture( "colorMapTexture", g_Obj[obj].m_Mesh.m_pTextures[i] ) )
                    else
                        V( g_pEffect->SetTexture( "colorMapTexture", g_pTexDef ) )
        ...
        for( UINT p = 0; p < cPass; ++p )
        {
            ...
            for( DWORD i = 0; i < g_LightMesh.m_dwNumMaterials; ++i )
            {
                ...
                V( g_pEffect->SetVector( "material", &vDif ) );
                V( g_pEffect->SetTexture( "colorMapTexture", g_LightMesh.m_pTextures[i] ) );
                V( g_pEffect->CommitChanges() );
                V( pMesh->DrawSubset( i ) );
            }
            V( g_pEffect->EndPass() );
...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
    ...
    V( g_pEffect->SetTexture( "shadowMapTexture", g_pShadowMap ) );
    ...
    V( g_pEffect->SetFloat("xMaxDepth", 60.0f));
    V( g_pEffect->SetMatrix( "lightViewProjMatrix", &mViewToLightProj ) );
    ...
    g_pEffect->SetTexture( "shadowMapTexture", NULL );
}
...
void RenderText()
{
    ...
    if( g_bShowHelp )
    {
        ...
        swprintf_s(text,L"Rotate camera\nMove camera\n"
            L"Rotate light\nMove light\n"
            L"Change view reference (Current: %s)\n" //Change light mode (Current: %s)\n
            L"Hide help\nQuit",
