...
public class ShadowCascade {
    ...
    public void update(Matrix4f viewMatrix, SunLight light) {

    	// Build projection view matrix for this cascade
        float aspectRatio = Game.getScene().getCamera().getAspect();
        projViewMatrix.setPerspective(Game.getScene().getCamera().getFOV(), aspectRatio, zNear, zFar);
        ...
        Vector3f lightDirection = new Vector3f().set(light.getDirection());
    ...
    private void updateLightViewMatrix(Vector3f lightDirection, Vector3f lightPosition) {
        ...
        lightViewMatrix.rotateX(MyMath.toRadians(lightAngleX))
        		.rotateY(MyMath.toRadians(lightAngleY))
        		.translate(-lightPosition.x, -lightPosition.y, -lightPosition.z);
        */
        lightViewMatrix.identity();
        lightViewMatrix.lookAt(lightDirection, lightPosition, GS.Y_AXIS);
    ...
    private void updateLightProjectionMatrix() {
        ...
        orthoProjMatrix.setOrtho(minX, maxX, minY, maxY, -distz*3, distz*3);
